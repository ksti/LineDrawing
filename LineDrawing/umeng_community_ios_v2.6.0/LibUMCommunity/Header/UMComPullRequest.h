...
@property (nonatomic, readonly) BOOL canReadNextPage;        //获取能不能访问下一页的字段(visit字段)0 - 代表需要登陆才能看到
...
/**
 获取附近的Users,使用方法见父类`UMComPullRequest`,获取的数据是`UMComUser`对象组成的NSArray
 
 */
@interface UMComUserNearbyRequest : UMComPullRequest
 ...
 *  since version 2.5
 */
@interface UMComPointDetailRequest : UMComPullRequest
...
@interface UMComPointOperationRequest : UMComPullRequest
 ...
 */
- (id)initWithPoint:(NSInteger)point
               desc:(NSString*)desc
 ...
 *  since version 2.5
 */
@interface UMComCurrencyDetailRequest : UMComPullRequest
