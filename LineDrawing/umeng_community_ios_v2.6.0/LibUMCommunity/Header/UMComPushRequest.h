...
/**
 ...
 @discuss 如果2.5.0之前使用self_account或other类型，请使用下面的loginWithCustomAccountForUser:completion:接口
 */
...
/**
 ...
 @param result 返回结果, responseObject是`UMComUser`对象,即登录成功之后返回的登录用户
 */
+ (void)loginWithCustomAccountForUser:(UMComUserAccount *)userAccount
                           completion:(PostResponseResultResponse)result;


/**
 更新登录用户数据(2.5版本之后不能通过这个接口更新用户头像， 更新头像请看下一个接口)
...
+ (void)updateUserIcon:(id)icon
              response:(PostResultResponse)result;
 ...
 *  @discuss 返回的时字典类型
 *  {
 *   'community_guest_open': int, #(0:没有开通访客模式，1:开通访客模式)
 *  }
 *
 *  @param response 回调block
 */
+(void)getCommunityGuestWithResponse:(PostResponseResultResponse)response;



+(void)getCommunityTokenWithAppkey:(NSString *)appKey
                         appSecret:(NSString *)appSecret
                          response:(PostResponseResultResponse)response;
