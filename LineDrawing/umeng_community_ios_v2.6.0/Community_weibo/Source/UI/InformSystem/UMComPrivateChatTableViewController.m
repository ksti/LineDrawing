...
- (void)refreshScrollViewDidScroll:(UIScrollView *)refreshScrollView haveNextPage:(BOOL)haveNextPage
{
    if (self.haveNextPage) {
        if (refreshScrollView.contentOffset.y < -150) {
            [self.refreshControl setAttributedTitle:[[NSAttributedString alloc] initWithString:UMComLocalizedString(@"um_com_loadingFingerUP", @"松手即可加载")]];
        }else if (refreshScrollView.contentOffset.y < 0){
            [self.refreshControl setAttributedTitle:[[NSAttributedString alloc] initWithString:UMComLocalizedString(@"um_com_pullDownLoadHistoryFeed", @"下拉可以加载历史聊天记录")]];
        }
    }else{
           [self.refreshControl setAttributedTitle:[[NSAttributedString alloc] initWithString:UMComLocalizedString(@"um_com_last_Page", @"已经是最后一页了")]];
    }
    self.loadMoreStatusView.haveNextPage = YES;
    //上拉
