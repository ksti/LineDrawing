...
-(void) initSubViews
{
    ...
    self.createTime.textColor = UMComColorWithColorValueString(UMCom_Micro_Feed_CreateTimeCollor);
    ...
    self.loacationName.textColor = UMComColorWithColorValueString(UMCom_Micro_Feed_LocationCollor);
...
- (BOOL)UMComWebView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (topicID) {
        ...
        if (umComTopic) {
            if (self.delegate && [self.delegate respondsToSelector:@selector(customObj:clickOnTopic:)]) {
                [self.delegate customObj:self clickOnTopic:umComTopic];
            }
        }
    }
    else if (uuid){
