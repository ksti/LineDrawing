...
- (void)reloadDetaiViewWithFeedStyle:(UMComFeedStyle *)feedStyle viewWidth:(CGFloat)viewWidth
{
    ...
    float totalHeight = self.nameLabel.frame.origin.y + self.nameLabel.frame.size.height+UMCom_Micro_FeedCellItem_Vertical_Space + self.cellSubViewTopEdge;
...
//        totalHeight += UMCom_Micro_FeedCellItem_Vertical_Space/2;
    ...
    if ([feed.distance integerValue] >= 0) {
        [self.locationDistance setText:[NSString stringWithFormat:@"%@",distanceString(feed.distance)]];
...
-(void)layoutMedalViews
{
    ...
    for (int i = 0; i < self.curMedalCount; i++) {
        ...
        medalView.backgroundColor = [UIColor clearColor];
    }
    //计算nameLabel的宽度
    CGFloat nameLabelWidth = 0;
    CGFloat nameLabelMaxWidth = self.feedStyle.nameLabelWidth  - totalMedalWidth - 10;
    if (self.feedStyle.isDetailFeed == YES) {
        nameLabelMaxWidth -= 20;
    }
    CGSize nameLabelSize = [self.nameLabel.text sizeWithFont:self.nameLabel.font constrainedToSize:CGSizeMake(nameLabelMaxWidth, self.nameLabel.frame.size.height)];
    if (nameLabelSize.width > nameLabelMaxWidth) {
