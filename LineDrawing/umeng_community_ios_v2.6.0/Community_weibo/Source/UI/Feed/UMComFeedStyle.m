...
- (instancetype)initWithDetailFeed:(UMComFeed *)feed viewWidth:(float)viewWidth
{
    self = [super init];
    if (self) {
        self.subViewDeltalWidth = 10;
        self.subViewOriginX = 10;
        self.nameLabelWidth = viewWidth-self.subViewOriginX-UMCom_Micro_Feed_More_ButtonWidth;
        self.subViewWidth = viewWidth - self.subViewOriginX*2;
        self.isDetailFeed = YES;
    }
    return self;
}
...
- (void)resetWithFeed:(UMComFeed *)feed
{
    ...
    if (origin_feed) {
        ...
        if ([feed.status intValue] >= FeedStatusDeleted ) {
            origin_feed.text = UMComLocalizedString(@"um_com_deleteContent", @"该内容已被删除");
        }
        ...
        if ([feed.status intValue] >= FeedStatusDeleted) {
            ...
            if (feed.origin_feed.text && [feed.origin_feed.text isEqualToString:@""] && (feed.origin_feed.image_urls.count > 0)) {
                ...
                [oringFeedString appendFormat:OriginUserNameString,originUserName,UMComLocalizedString(@"um_com_shareImage", @"分享图片")];
            }
