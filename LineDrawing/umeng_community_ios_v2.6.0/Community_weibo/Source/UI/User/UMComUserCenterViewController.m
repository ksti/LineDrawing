...
- (void)creatChildViewControllers
{
    ...
    postTableViewController.isShowEditButton = NO;
    ...
    UMComUserTableViewController *followersTableViewController = [[UMComUserTableViewController alloc] initWithFetchRequest:[[UMComFollowersRequest alloc] initWithUid:self.user.uid count:BatchSize]];
    ...
    UMComUserTableViewController *fanTableViewController = [[UMComUserTableViewController alloc]initWithFetchRequest:[[UMComFansRequest alloc] initWithUid:self.user.uid count:BatchSize]];
...
- (void)userProfileDetailView:(UMComUserProfileDetailView *)userProfileDetailView clickOnfocuse:(UIButton *)focuseButton
{
    ...
    [UMComLoginManager performLogin:self completion:^(id responseObject, NSError *error) {
        if (!error) {
            BOOL isFollow = !([weakSelf.user.relation integerValue] == 1 || [weakSelf.user.relation integerValue] == 3);
            [UMComPushRequest followerWithUser:weakSelf.user isFollow:isFollow completion:^(NSError *error) {
