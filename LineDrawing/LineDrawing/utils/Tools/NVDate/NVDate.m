...
- (instancetype)nextHour {
    return [self nextHours:1];
}

- (instancetype)nextHours:(NSInteger)hours {
    return [self hoursCalculate:hours isForward:YES];
}

- (instancetype)hoursCalculate:(NSInteger)hours isForward:(BOOL)isForward {
    NSDateComponents *dateComponents = [[NSDateComponents alloc] init];
    dateComponents.hour = hours * (isForward ? 1 : -1);
    _date = [_calendar dateByAddingComponents:dateComponents toDate:_date options:0];
