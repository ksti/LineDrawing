...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:frame])) {
        ...
        longPressGr.allowableMovement = 5.0f;
...
-(void)messageShow:(NSString *)mess
{
    UILabel *alertLAbel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 160, 40)];
    alertLAbel.backgroundColor = [UIColor blackColor];
    [self addSubview:alertLAbel];
    alertLAbel.center = CGPointMake(self.frame.size.width/2, self.frame.size.height-100);
    ...
    alertLAbel.layer.cornerRadius = 2.0f;
    
    alertLAbel.textColor = [UIColor whiteColor];
    alertLAbel.font = [UIFont systemFontOfSize:14];
    
    alertLAbel.text  = mess;
    alertLAbel.textAlignment = NSTextAlignmentCenter;
    NSDictionary *dic = [NSDictionary dictionaryWithObjectsAndKeys:alertLAbel,@"alertLabel", nil];
    [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(hiddenAlertLabel:) userInfo:dic repeats:NO];
}
-(void)hiddenAlertLabel:(id)object
{
    UILabel *label = (UILabel*)[[object userInfo ]objectForKey:@"alertLabel"];
